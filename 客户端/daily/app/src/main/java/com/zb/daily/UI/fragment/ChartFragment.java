...
public class ChartFragment extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        dateMonth = new SimpleDateFormat("yyyy").format(new Date());
        dateTextView = activity.findViewById(R.id.fragment_chart_date);
        dateTextView.setText(dateMonth);

        mBarChart = activity.findViewById(R.id.fragment_chart_pie);
        mBarChart2 = activity.findViewById(R.id.fragment_chart_pie2);
        BarChartInit(mBarChart,1,dateMonth);
        BarChartInit(mBarChart2,2,dateMonth);
    ...
    private void BarChartInit(BarChart BarChart, int type, String month) {
        BarChart.setExtraOffsets(5, 10, 5, 5);
        mBarData = getBarData(type, month);
        showBarChart(BarChart, mBarData);
    }

    private BarData getBarData(int type, String month) {
        ...
        for (int day = 1; day <= 12; day++) {
            ...
            float summary = (float) recordDao.getMonthSummary(month + "-" + dayStr, type);
            Map<String, Double> map = recordDao.getMonthSummaryByCategory(month, type);
            //模拟数据


            yValues.add(new BarEntry(day, summary));
